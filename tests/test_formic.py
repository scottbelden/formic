...
from builtins import object, str, range, zip
...
from formic import get_version, MatchType, Matcher, ConstantMatcher, FNMatcher, \
    ...
    get_path_components, reconstitute_path, walk_from_list
import pytest
import os
...
class TestMiscellaneous(object):
    ...
    def test_search_prune_efficiency(self, tmpdir):
        test_dir = tmpdir.mkdir('test')
        lower_dir = test_dir.mkdir('lower')
        lower_dir.join('lower.txt').write('')

        another_dir = tmpdir.mkdir('another')
        yet_another_dir = another_dir.mkdir('yet_another')
        yet_another_dir.join('yet_another.txt').write('')

        print("Absolute, starting at ", tmpdir)
        rooted = FileSet(include="/test/lower/lower.txt", directory=str(tmpdir), default_excludes=False)
        files = [f for f in rooted]
        assert len(files) == 1

        floating = FileSet(include="/*/lower/lower.txt", directory=str(tmpdir), default_excludes=False)
        files = [f for f in floating]
        assert len(files) == 1
        assert rooted._received < floating._received

    def test_filename_case(self, tmpdir):
        test_dir = tmpdir.mkdir('test')

        lower_dir = test_dir.mkdir('lower')
        lower_dir.join('lower.txt').write('')
        lower_dir.join('UPPER.txt').write('')

        upper_dir = test_dir.mkdir('UPPER')
        upper_dir.join('lower.txt').write('')
        upper_dir.join('UPPER.txt').write('')

        test_dir.join('formic.py').write('')
        test_dir.join('VERSION.txt').write('')

        for test in ["/lower/lower.txt", "lower/UPPER.txt", "UPPER/lower.txt", "UPPER/UPPER.txt"]:
            print("Testing", test)
            found = [f for f in FileSet(include=test, directory=str(test_dir))]
            assert len(found) == 1
            print("   ... found", test)
