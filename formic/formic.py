...
def get_version():
    ...
    try:
        ...
        value = resource_string(__name__, "VERSION.txt")
    except:
...
class MatchType(object):
    ...
    starting ``BIT_``."""
    BIT_MATCH              = 1  # M  # noqa
    BIT_ALL_SUBDIRECTORIES = 2  # A  # noqa
    BIT_NO_SUBDIRECTORIES  = 4  # N  # noqa

    # The Match types             -BIT FIELDS-
    #                             X  M   A   N
    NO_MATCH                    = 0             # noqa
    MATCH                       =    1          # noqa
    MATCH_ALL_SUBDIRECTORIES    =    1 | 2      # noqa
    MATCH_BUT_NO_SUBDIRECTORIES =    1     | 4  # noqa
    NO_MATCH_NO_SUBDIRECTORIES  =            4  # noqa
        ...
        for element in elements:
            if element == "..":
                raise FormicError("Invalid glob:"
                                  " Cannot have '..' in a glob: {0}".format("/".join(elements)))
            elif element == ".":
...
def get_initial_default_excludes():
    ...
    return [Pattern.create(exclude) for exclude in excludes]
...
class FileSet(object):
    ...
    * You can provide and alternate function to :func:`os.walk()` that, for
    ...
    def __str__(self):
        return "FileSet [directory={0}, include={1}, exclude={2}, symlinks? {3}]".format(
            self.directory if self.directory else "CWD ({0})".format(getcwd()),
